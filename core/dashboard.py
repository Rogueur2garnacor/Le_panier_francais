...
def get_total_items() -> int:
    try:
        ...
        print(f"L'erreur dashboard 1.1 s'est produite: {e}")
...
def get_low_stock_items() -> list:
    try:
        ...
        print(f"L'erreur dashboard 1.2 s'est produite: {e}")
...
def get_most_used_items_in_recipes() -> list:
    try:
        ...
        print(f"L'erreur dashboard 1.3 s'est produite: {e}")
...
def get_total_recipes() -> int:
    try:
        ...
        print(f"L'erreur dashboard 1.4 s'est produite: {e}")
...
def get_most_popular_recipes() -> list:
    try:
        ...
        print(f"L'erreur dashboard 1.5 s'est produite: {e}")
    


def get_most_frequent_shopping_list_items() -> list:
    try:
        """Retourne une liste des articles les plus fréquemment ajoutés aux listes de courses."""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
            sql = """
            ...
            print(f"Erreur lors de la récupération des articles fréquents : {e}")
            return []
        finally:
            conn.close()
    except Exception as e:
        print(f"L'erreur dashboard 1.6 s'est produite: {e}")
...
def get_weekly_menu() -> list:
    try:
        ...
        print(f"L'erreur dashboard 1.7 s'est produite: {e}")